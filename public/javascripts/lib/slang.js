(function() {
    ...
    slang.plural = function plural(input) {
      return input.charAt(input.length - 1) === 's' ? input : input.concat('s');
    }
    
    // Returns a singular version of a word
    slang.singular = function singular(input) {
      return input.charAt(input.length - 1) === 's' ? slang.chop(input) : input;
    }
